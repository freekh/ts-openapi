...
interface ApiExecutor<ApiEndpoints> {
  exec<Path extends Paths>(method: Method, path: Path, params: any): ApiEndpoints
}

interface ApiEndpointFormatter {

}

class AxiosExecutor<A> implements ApiExecutor<A> {
  config: AxiosRequestConfig
  constructor(config: AxiosRequestConfig, formatter?: ApiEndpointFormatter) {
    this.config = config
  }
  exec<Path extends Paths>(method: Method, path: Path, params: any): A<Path> {
    ...
    }).then(formatter.format(method, path))
  }
}

function api(apiExecutor: ApiExecutor<>) {
  return function<Path extends Paths>(path: Path): ApiEndpoint<Path> {
    switch(path) {
      case '/article/conversions':
        return {
          get: (id: string[]) => apiExecutor.exec('get', path, { id })
